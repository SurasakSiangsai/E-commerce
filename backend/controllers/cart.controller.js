...
export const getCartProducts = async (req, res) => {
	try {
		const products = await Product.find({ _id: { $in: req.user.cartItems.map((item) => item.product) } })
			.populate("createdBy", "name"); // Populate seller's name

		// Add quantity and seller name for each product
		const cartItems = products.map((product) => {
			const item = req.user.cartItems.find((cartItem) => cartItem.product.toString() === product._id.toString());
			return {
				...product.toJSON(),
				quantity: item.quantity,
				sellerName: product.createdBy?.name || "N/A", // Include seller's name
			};
		});
